...
pub fn init_world(testbed: &mut Testbed) {
    ...
    let shape_collide = SharedShape::capsule_y(1.0, 1.0);
    ...
    if let Ok(Some(contact)) = parry::query::contact(
        ...
        let rigid_body = RigidBodyBuilder::fixed()
            .translation(contact.point1.coords);
        let handle = bodies.insert(rigid_body);
        colliders.insert_with_parent(ColliderBuilder::new(shape_collide.clone()), handle, &mut bodies);
